...
(McpToolTest, McpDispatcherTest, McpTransportTest, McpContractTest) cover the Grail-free server.
Uses valid Python and a transpile-time semantic error only -- Python *syntax* and *runtime* errors
crash the gem, so those two paths are switched-off tripwires gated by class-side flags.
...
expectvalue /Class
doit
McpGrailToolsetTest category: 'MCPServer'
%
...
mcp
  "A fresh Grail TOOLSET whose tool_* handlers we exercise directly (no socket, no dispatcher).
   Attached to a server because the handlers use its shared output cap (capResult:)."
  ^McpGrailToolset on: McpServer new
%
category: 'helpers'
method: McpGrailToolsetTest
