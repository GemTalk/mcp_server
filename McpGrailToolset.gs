...
'The optional GemStone-Python (Grail) tools: eval_python and compile_python. Kept in a file that is
loaded only into a Grail-equipped image (see load-grail.gs), because a method referencing ModuleAst
cannot be compiled in an image without it -- which is precisely why this is a separate FILE and not
part of the Mcp-Core package.
...
McpServer class>>installedDefaultToolsetNames once this file is loaded, or named explicitly in a
router''s toolsetNames.

Unlike the core toolsets this one owns its handlers (see McpToolset), so it also serves as the worked
example for a third-party toolset. NOT read-only-safe: readOnlySafeToolNames is inherited (empty), so
a read-only session drops both tools -- running arbitrary Python can persist anything.
...
No ModuleAst capability check is performed: this file only loads into an image that has Grail.'
%
expectvalue /Class
doit
McpGrailToolset category: 'MCPServer'
%
...
registerOn: aToolRegistry
  "Register the two python tools. These require an image with GemStone-Python (Grail/ModuleAst)
   whose parser raises exceptions on syntax errors rather than crashing the gem; no capability
   check is performed."
  | codeArg |
...
tool_compile_python: args
  "Transpile Python source to Smalltalk via Grail and answer the generated source. capResult: is the
   server's shared 50k output cap; Python errors become #pythonError (withPythonErrorsAsMcpError:)."
  ^self withPythonErrorsAsMcpError: [
    server capResult: (ModuleAst parseSource: (args at: 'code')) smalltalkSource]
%
category: 'tools - python'
method: McpGrailToolset
tool_eval_python: args
  "Evaluate Python source via Grail and answer the printString of the result. capResult: is the
   server's shared 50k output cap; Python errors become #pythonError (withPythonErrorsAsMcpError:)."
  ^self withPythonErrorsAsMcpError: [
    server capResult: (ModuleAst evaluateSource: (args at: 'code')) printString]
%
