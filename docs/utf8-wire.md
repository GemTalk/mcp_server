...
Two things cover it now. `McpUtf8Test>>testWorkerDecodesABodyItRecompiled` logs a real worker gem
in and drives an accented body through `McpSession>>forward:`, so the real expression builder and a
real compile of the literal are both in the path — the gap the bug lived in. It costs one session
slot, and it is the only in-image test that crosses that boundary. And `test.sh` grew a Unicode
section, over a real socket through a real GCI hop, which is the only place either half of this can
be seen.
...
- the worker → front-end hop is measured in bytes by the kernel's result fetch, whose buffer is
  sized in bytes (see `McpSession class>>resultBufferBytes` and the #51438 cover that rides on it);
