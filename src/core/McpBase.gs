...
parseBody: aString
  "Parse a JSON-RPC request body to its Dictionary, or nil if empty/malformed. A valid JSON-RPC
   ...
   decodes cannot disagree. It follows that aString must be a byte String, which is what a socket
   read and gs-mcp''s own ASCII JSON both are; a wide string would not understand #decodeFromUTF8.
