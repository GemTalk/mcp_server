...
testWorkerDecodesUtf8AndAnswersAscii
  "The worker entry's Unicode contract, at the boundary a client's bytes actually cross.
   ...
   McpBase>>parseBody: it is read one Latin-1 character per byte, and the corruption goes on to be
   stored -- which is why that decode is the one Unicode fix gs-mcp still carries.
