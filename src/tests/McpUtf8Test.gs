...
McpBase>>parseBody:, which is `JsonParser parse: aString decodeFromUTF8 asString`.
...
testAsciiBodyCostsNothingToDecode
  ...
  parsed := McpServer new parseBody: body.
...
testMalformedUtf8RefusesTheWholeBody
  ...
  | srv truncated bareContinuation overlong encodedSurrogate |
  srv := McpServer new.
  ...
  self assert: (srv parseBody: truncated) isNil.
  self assert: (srv parseBody: bareContinuation) isNil.
  self assert: (srv parseBody: overlong) isNil.
  self assert: (srv parseBody: encodedSurrogate) isNil.
...
testMultiByteSequencesAreDecoded
  ...
  | parsed |
  parsed := McpServer new parseBody:
  ...
  parsed := McpServer new parseBody:
    (self bodyOfBytes: #(123 34 107 34 58 34 16rE2 16r98 16r83 34 125)).
...
testParsedKeysCompareWithStringLiterals
  ...
  | parsed |
  parsed := McpServer new parseBody: (self bodyOfBytes:
