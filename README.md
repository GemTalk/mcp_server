...
src/tests/   18 classes  the SUnit suites and their fixtures                          (always)
...
socket-less suites `McpToolTest` (52), `McpDispatcherTest` (12), `McpSessionTest` (14),
`McpOutboxTest` (9), `McpStreamTest` (18), `McpLifetimeTest` (43), `McpTransportTest` (32),
`McpContractTest` (34) and `McpExtensionTest` (9), plus `McpExternalSessionTest` (5) and
`McpWorkerDeadlineTest` (4) — **232 tests**,
which is the whole suite on a base install. Where the optional groups are installed the runner picks
their suites up automatically: plus `McpAuthTest` (31) and `McpAuthConformanceTest` (25) — **288
tests** — and **297 with the 9 in `McpGrailToolsetTest`** on a Grail image.

Four suites are not purely in-image and need a **netldi** running. `McpAuthTest` and
...
the worker sent, and `McpWorkerDeadlineTest` that a call which outruns the request deadline is
really broken in a real one — so the runner asks for a netldi on any image where they are installed,
which is every image, since both are part of the base install. That is no new burden in practice: gs-mcp gives every
