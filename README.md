...
`--grail` (or `GS_MCP_WITH_GRAIL=1`) files in `load-grail.gs` instead, which adds the `src/grail/`
group on top. `run-server.sh` builds a base `McpRouter` instance and calls
...
load-grail.gs            files in core + tests + grail, then commits
```
...
> image, while the identical file-in into a Rowan-free image loaded all 28 classes with no compiler
