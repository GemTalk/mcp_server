...
# an image with kernel JWT support and src/grail only on --grail. A base install runs the seven core
# suites.
#
...
classes := #( 'McpToolTest' 'McpDispatcherTest' 'McpSessionTest' 'McpTransportTest'
  'McpContractTest' 'McpExtensionTest' 'McpExternalSessionTest' ) asOrderedCollection.
