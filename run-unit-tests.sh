...
# McpWorkerDeadlineTest (both always installed, see below) and McpAuthTest (only where the auth
# group could be). Ask the image which are present
...
# through a suite run.
#
# In practice the first two are always there, so a netldi is in practice always required.
