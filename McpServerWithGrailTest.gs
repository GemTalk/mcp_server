set compile_env: 0
! ------------------- Class definition for McpServerWithGrailTest
expectvalue /Class
doit
GsTestCase subclass: 'McpServerWithGrailTest'
  instVarNames: #()
  classVars: #()
  classInstVars: #()
  poolDictionaries: #()
...
expectvalue /Class
doit
McpServerWithGrailTest comment:
'Tests for the optional Grail-powered python tools on McpServerWithGrail (eval_python /
compile_python). Loaded only on a Grail-equipped image (see load-grail.gs); the base suites
(McpToolTest, McpDispatcherTest, McpTransportTest) cover the Grail-free server. Uses valid
...
expectvalue /Class
doit
McpServerWithGrailTest category: 'MCPServer'
%
! ------------------- Remove existing behavior from McpServerWithGrailTest
removeallmethods McpServerWithGrailTest
removeallclassmethods McpServerWithGrailTest
! ------------------- Class methods for McpServerWithGrailTest
category: 'enablement'
classmethod: McpServerWithGrailTest
...
pythonSyntaxErrorsThrow
  ...
  ^false
%
! ------------------- Instance methods for McpServerWithGrailTest
category: 'helpers'
...
dispatch: requestDict
  ...
  ^(McpDispatcher withToolRegistry: McpServerWithGrail new toolRegistry) handle: requestDict
%
category: 'helpers'
method: McpServerWithGrailTest
...
testToolsCallWrapsPythonRuntimeErrorAsIsError
  "Tripwire for the day Grail stops crashing on a Python *runtime* exception. Guarded by
   McpServerWithGrailTest class>>pythonRuntimeErrorsThrow (currently false), so today it
...
testToolsCallWrapsPythonSyntaxErrorAsIsError
  "Tripwire for the day Grail stops crashing on malformed Python. Guarded by
   McpServerWithGrailTest class>>pythonSyntaxErrorsThrow (currently false), so today it
